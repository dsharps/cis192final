...
TwitterStats will pull as many as 3,200 tweets from Twitter’s (the maximum allowable by the twitter API), so it might take a second to load the info page. The function to get all those tweets is pretty interesting - we had to do it in many iterations to work around the tweet download cap. We also have a pretty slick curseword counting function - check it out!